...
class ReceiptRepository extends ServiceEntityRepository
{
    ...
    public function findPulic(?int $maxResultCount){

        $fetch=$this->createQueryBuilder('r')
        ->orderBy('r.name')
        ->where('r.isPublic = 1');
        
        if(is_integer($maxResultCount) && $maxResultCount>0){
            $fetch->setMaxResults($maxResultCount);
        }

        return $fetch->getQuery()->getResult();
    }
